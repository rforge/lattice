...
\Rfunction{coef}.  More generally, for multipanel plots the current
panel will represent some subset of the full dataset, and we would
need to somehow figure out (inside the panel function) the
corresponding sub-part of the model to plot.  For example, here we
needed to figure out which \code{Subject} was represented in the
panel.
...
\code{fitted(fit)}.  This approach works for all modeling functions
that support a \Rfunction{fitted} method, although one should be
careful to check how \code{NA} values are handled.
\\
...
@
%
We leave to the reader as an exercise the task of figuring out why
this works (Hint: see \code{help(panel.superpose)}).
...
\subsection*{GCSE exam scores revisited}

Trying the same approach with the \code{Gcsemv} data unfortunately
fails (even after we account for a length mismatch between the results
of \Rfunction{fitted} and the original data due to missingness).
<<>>=
...
@
%
This happens because unlike in the previous example, the $x$ values
are not ordered within each panel.  Even if we could take care of that
problem, the $x$ are not equispaced, and are in fact much more densely
packed in certain regions than necessary.

\newpage


As before, we could use a built-in model supported by
\Rfunction{panel.xyplot}, such as
<<>>=
xyplot(written ~ course | gender, Gcsemv, type = c("p", "r"), col.line = "black")
@
<<echo=FALSE,fig=TRUE,width=9,height=4.5>>=
plot(trellis.last.object())
@
%
or the more complicated (but still restricted to use within-panel data)
<<>>=
xyplot(written ~ course | gender, data = Gcsemv,
       panel = function(x, y, ...) {
           fm <- lm(y ~ x + I(x^2))
           panel.xyplot(x, y, ...)
           xx <- do.breaks(range(x, finite = TRUE), 100)
           panel.lines(xx, predict(fm, list(x = xx)), col = "black")
       })
@
%
<<echo=FALSE,fig=TRUE,width=9,height=4.5>>=
...
More complex models need a little more work.  Let us Consider the
...
@
%
Our goal is to compare \code{fm2} and \code{fm1} (which incorporate
\code{gender} as a factor first additively and then as an interction)
with \code{fm0}.  
...
\item Often modeling and data manipulation should be done prior to
  plotting step.
