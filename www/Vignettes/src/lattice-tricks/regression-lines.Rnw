...
\title{Displaying Hello World model fits in Lattice plots}
...
arguments to be upplied to it.  Arguments that are not recognized by
...
function examples that did nothing special.  Following our arguments
in reverse, we now see that the above is equivalent to
<<>>=
...
making this a very useful approach.
