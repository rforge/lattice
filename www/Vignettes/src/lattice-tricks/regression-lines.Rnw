...
data and plot them along with the data.
\\
...
are simple \R\ functions (!).  They are responsible for drawing the
...
function.  To make things a little more complicated, but keep the
results unchanged, we can explicitly define a new inline function,
which simply calls \Rfunction{panel.xyplot} with exactly the arguments
given to it.
<<eval=FALSE>>=    
xyplot(written ~ course | gender, data = Gcsemv, 
       xlab = "Coursework score", 
