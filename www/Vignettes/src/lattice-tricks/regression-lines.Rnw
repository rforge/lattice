...
\lattice\ is an add-on package for \R\ that implements Trellis
...
~~~~This article discusses the common situation where we want to augment
...
to it.\footnote{The mysterious \code{\dots} needs some explanation,
